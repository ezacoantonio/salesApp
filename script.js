...
inventory.forEach(category => {
  ...
  category.items.forEach(item => {
    ...
    const initialLoad = document.createElement("input");
    initialLoad.type = "number";
    initialLoad.id = item.name + "Initial";
    initialLoad.placeholder = "Initial load of " + item.name;
    itemDiv.appendChild(initialLoad);

    const finalLoad = document.createElement("input");
    finalLoad.type = "number";
    finalLoad.id = item.name + "Final";
    finalLoad.placeholder = "Final load of " + item.name;
    itemDiv.appendChild(finalLoad);
...
function calculateInventory() {
  ...
  inventory.forEach(category => {
    category.items.forEach(item => {
      const initial = parseFloat(document.getElementById(item.name + "Initial").value);
      const final = parseFloat(document.getElementById(item.name + "Final").value);
      const price = parseFloat(document.getElementById(item.name + "Price").value);

      if (initial < final) {
        alert('Initial inventory of ' + item.name + ' cannot be less than final inventory. Please check the inputs.');
        return;
      }

      const quantitySold = initial - final; 
